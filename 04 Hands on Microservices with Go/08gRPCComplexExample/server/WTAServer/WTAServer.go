...
import (
	...
	repo "hands-on-microservices/08gRPCComplexExample/server/repository"
...
func (s *MyWTAServer) GetPlayerWithHighestRanking(ctx context.Context, in *pb.PlayerIdRequest) (*pb.PlayerWithRanking, error) {
	result, err := s.Repo.GetPlayerWithHighestRanking(in.GetPlayerId())
	if err != nil {
		return nil, err
	}
...
//GetPlayers stream is read and write
func (s *MyWTAServer) GetPlayers(stream pb.WTA_GetPlayersServer) error {
	...
	for {
		...
		log.Printf("Recieved request for PlayerIDDD: &d", in.GetPlayerId())
